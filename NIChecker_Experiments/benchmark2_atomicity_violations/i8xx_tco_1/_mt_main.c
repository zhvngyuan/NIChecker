...
//#define spin_lock(l) do { while (l) { /* spin */  } } while(false)
void spin_lock(int);
...
static unsigned char __init i8xx_tco_getdevice (void)
{
	...
	if (i8xx_tco_pci) {
		...
		if (badr == 0x0001 || badr == 0x0000) {
			printk (KERN_ERR PFX "failed to get TCOBASE addressn");
		...
		if (val1 & 0x02) {
			...
			pci_read_config_byte (i8xx_tco_pci, 0xd4, &val1);
			if (val1 & 0x02) {
				printk (KERN_ERR PFX "failed to reset NO_REBOOT flag, reboot disabled by hardwaren");
				pci_dev_put(i8xx_tco_pci);
		...
		if (!request_region (SMI_EN + 1, 1, "i8xx TCO")) {
			printk (KERN_ERR PFX "I/O address 0x%04x already in usen",
				SMI_EN + 1);
			pci_dev_put(i8xx_tco_pci);
			return 0;
		}
...
static int __init watchdog_init (void)
{
	...
	if (tco_timer_set_heartbeat (heartbeat)) {
		...
		printk(KERN_INFO PFX "heartbeat value must be 2<heartbeat<39, using %dn",
			heartbeat);
	}

	ret = register_reboot_notifier(&i8xx_tco_notifier);
	if (ret != 0) {
		printk(KERN_ERR PFX "cannot register reboot notifier (err=%d)n",
			ret);
		goto unreg_region;
	}
	...
	printk (KERN_INFO PFX "initialized (0x%04x). heartbeat=%d sec (nowayout=%d)n",
...
void *closer1_isr(void *arg) {
    //while(cnt2<LIMIT) {
	  if(test_global_condition3 == 1){ 
        ...
        i8xx_tco_release();
        cnt2++;
	  if(test_global_condition4 == 1)test_global_array[9000] = 999;
    //}
}
//priority 2
