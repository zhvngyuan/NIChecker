...
#endif
void wdt_ctr_mode(int ctr, int mode) {
  do { 
    ctr * 2 * 2 * 2 * 2 * 2 * 2; 
...
#endif 
void wdt_ctr_load(int ctr, int val) {
  do { 
...
static irqreturn_t wdtpci_interrupt(int irq, void *dev_id)
{
	...
	printk(KERN_CRIT PFX "status %dn", status);

#ifdef CONFIG_WDT_501_PCI
	if (!(status & WDC_SR_TGOOD))
 		printk(KERN_CRIT PFX "Overheat alarm.(%d)n",inb_p(WDT_RT));
	if (!(status & WDC_SR_PSUOVER))
 		printk(KERN_CRIT PFX "PSU over voltage.n");
	if (!(status & WDC_SR_PSUUNDR))
 		printk(KERN_CRIT PFX "PSU under voltage.n");
	if (tachometer) {
		if (!(status & WDC_SR_FANGOOD))
			printk(KERN_CRIT PFX "Possible fan fault.n");
	}
#endif /* CONFIG_WDT_501_PCI */
	if (!(status&WDC_SR_WCCR))
#ifdef SOFTWARE_REBOOT
#ifdef ONLY_TESTING
		printk(KERN_CRIT PFX "Would Reboot.n");
#else
		printk(KERN_CRIT PFX "Initiating system reboot.n");
		emergency_restart(NULL);
#endif
#else
		printk(KERN_CRIT PFX "Reset in 5ms.n");
#endif
	return IRQ_HANDLED;
}
#endif
				   ...
				   const struct pci_device_id *ent)
{
	...
	dev_count++;
	if (dev_count > 1) {
		printk (KERN_ERR PFX "this driver only supports 1 devicen");
		return -ENODEV;
	}

	if (pci_enable_device (dev)) {
		printk (KERN_ERR PFX "Not possible to enable PCI Devicen");
		return -ENODEV;
	}

	if (pci_resource_start (dev, 2) == 0x0000) {
		printk (KERN_ERR PFX "No I/O-Address for card detectedn");
		ret = -ENODEV;
		goto out_pci;
	}
	...
	if (request_region (io, 16, "wdt_pci") == NULL) {
		printk (KERN_ERR PFX "I/O address 0x%04x already in usen", io);
		goto out_pci;
	}

	if (request_irq (irq, wdtpci_interrupt, IRQF_DISABLED | IRQF_SHARED,
			 "wdt_pci", &wdtpci_miscdev)) {
		printk (KERN_ERR PFX "IRQ %d is not freen", irq);
		goto out_reg;
	}

	printk ("PCI-WDT500/501 (PCI-WDG-CSM) driver 0.10 at 0x%04x (Interrupt %d)n",
		io, irq);

	/* Check that the heartbeat value is within it's range ; if not reset to the default */
	if (wdtpci_set_heartbeat(heartbeat)) {
		wdtpci_set_heartbeat(WD_TIMO);
		printk(KERN_INFO PFX "heartbeat value must be 0<heartbeat<65536, using %dn",
			WD_TIMO);
	}

	ret = register_reboot_notifier (&wdtpci_notifier);
	if (ret) {
		printk (KERN_ERR PFX "cannot register reboot notifier (err=%d)n", ret);
		goto out_irq;
	}
	...
	if (ret) {
		printk (KERN_ERR PFX "cannot register miscdev on minor=%d (err=%d)n",
			TEMP_MINOR, ret);
		goto out_rbt;
	}
#endif /* CONFIG_WDT_501_PCI */
	...
	printk(KERN_INFO PFX "initialized. heartbeat=%d sec (nowayout=%d)n",
		heartbeat, nowayout);
#ifdef CONFIG_WDT_501_PCI
	printk(KERN_INFO "wdt: Fan Tachometer is %sn", (tachometer ? "Enabled" : "Disabled"));
#endif /* CONFIG_WDT_501_PCI */
...
// add by zhangyuan
int test_global_array[MAX_LENGTH];
...
void writer1_isr(void) {
    //while (cnt3 < LIMIT) {
        ...
        cnt3++;
        if (test_global_condition3 == 1) test_global_var1 = 0x99;
        if (test_global_condition4 == 0) test_global_var2 = 0x99;
    //}
}
//priority 4
...
void writer3_isr(void) {
	...
	int i = 0;
      ...
      for (i = 0; i < MAX_LENGTH; i++) {
        if (i == TRIGGER) test_global_array[TRIGGER] = 1;
        if (i == MAX_LENGTH + 1) test_global_array[TRIGGER] = 1;
      }
    	mininum = test_global_array[TRIGGER] - 10;
